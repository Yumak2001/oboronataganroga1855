...
public class Hostels {
    ...
    public static ArrayList<Hostel> readHostelsJSONFile(Context context) throws IOException, JSONException {

        String jsonText = readText(context, R.raw.hostels);

        JSONArray jsonRoot = new JSONArray(jsonText);
        ArrayList<Hostel> hostels = new ArrayList<>(jsonRoot.length());
        for (int i = 0; i < jsonRoot.length(); i++) {
            ...
            hostels.add(new Hostel(type, title, discount, map1, map2));
        }
        return hostels;
    }
